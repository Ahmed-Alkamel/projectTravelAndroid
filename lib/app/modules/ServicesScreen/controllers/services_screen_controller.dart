...
class ServicesScreenController extends GetxController {
  ...
  loadDataBase() async {
    ...
    company.value = await companiesRepository!.allTrip();
  ...
  goToScreanBeneficiares(Trips trip) async {
    ...
    if (currentPlan.id != 4) {
      selectTrip.first.dateGo = trip.dateGo;
      selectTrip.first.idTrips = trip.idTrips;
      selectTrip.first.price = trip.price;
    }
    await loadServicesId(trip);
    if (currentUser.value != null && currentUser.value!.identityMe != null) {
      currentUser.value!.identityMe!.isSelected.value = false;
    }
  ...
  double convertCurrence(double price, Currencies currencie) {
    if (isCurrenc!.isDefaulte) {
      if (!currencie.isDefaulte) {
        return price * currencie.conversionRate * 100;
      } else {
        return price;
      }
    } else {
      if (currencie.isDefaulte) {
        return price / (isCurrenc!.conversionRate * 100);
      } else {
        var defaultcurrens = dataBasic.value!.currencies!
            .singleWhere((element) => element.isDefaulte);

        double covettoDefuat = price * currencie.conversionRate * 100;
        return covettoDefuat / (isCurrenc!.conversionRate * 100);
      }
    }
  }

  void updateTripUpdata() {
    update(['TripUpdata']);
  }
}
